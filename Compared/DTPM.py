...
class DTE():
    ...
    def fit(self, X_train, y_train=None, X_test=None, y_test=None, verbose=False):
        if self.model is None:  # allows retraining
            ...
            # summary(self.model,(X_train.shape[-1],), device='cuda')

            # from thop import profile
            # x = torch.randn([1,X_train.shape[1]])
            # flops, params = profile(self.model.to('cpu'), (x,))
...
class DTECategorical(DTE):
    def __init__(self, seed=0, model_name="DTE_categorical", hidden_size=[256, 512, 256], epochs=400, batch_size=64,
                 lr=1e-4, weight_decay=5e-4, T=400, num_bins=7):
        if num_bins < 2:
...
class DTEInverseGamma(DTE):
    def __init__(self, seed=0, model_name="DTE_inverse_gamma", hidden_size=[256, 512, 256], epochs=400, batch_size=64,
                 lr=1e-4, weight_decay=5e-4, T=400):
...
class DTEBagging():
    def __init__(self, num_bags=5, hidden_size=[256, 512, 256], epochs=200, batch_size=64, lr=1e-4, weight_decay=5e-4,
                 T=300, num_bins=7):
        self.hidden_size = hidden_size
        self.epochs = epochs
        self.batch_size = batch_size
    ...
    def fit(self, X_train, y_train, X_test, Y_test):
        for _ in range(self.num_bags):
            ...
            model = DTE(hidden_size=self.hidden_size, epochs=self.epochs, batch_size=self.batch_size, lr=self.lr,
                        weight_decay=self.weight_decay, T=self.T, num_bins=self.num_bins)
